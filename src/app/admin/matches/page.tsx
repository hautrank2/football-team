...
const AdminMatchesPage = () => {
  ...
  const dayDate = useMemo(
    () => (day ? parse(day, "yyyy-MM-dd", new Date()) : undefined),
    [day],
  );
  ...
  const existing = dayDate
    ? matches.find((m) => isSameDay(new Date(m.matchDate), dayDate))
    : undefined;
              ...
              <>
                <div className="rounded-md border">
                  <div className="border-b px-3 py-2 text-sm text-muted-foreground">
                    ...
                    {votes.length} người vote (
                    {votes.reduce((s, v) => s + 1 + v.guestCount, 0)} suất)
                  </div>
                ...
                <Button
                  onClick={onCreate}
                  disabled={votes.length === 0 || create.isPending}
                  className="w-fit"
                >
            ...
            matches.map((m) => (
              ...
              >
                <span className="capitalize">
                  {format(new Date(m.matchDate), "EEEE, dd/MM/yyyy", {
                    locale: vi,
                  })}
                </span>
                <span className="flex items-center gap-2">
                  {m.fieldCost == null ? (
                    <Badge variant="outline">Chưa nhập tiền</Badge>
                  ) : null}
